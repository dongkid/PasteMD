...
if TYPE_CHECKING:
    from ..presentation.webview import WebViewLauncher, WebViewManager
...
class Container:
    ...
    def __init__(self):
        ...
        # WebView 组件（延迟初始化）
        self._webview_launcher: "WebViewLauncher | None" = None
    ...
    def get_webview_launcher(self) -> "WebViewLauncher":
        """获取 WebView 启动器（延迟初始化）"""
        if self._webview_launcher is None:
            from ..presentation.webview import WebViewLauncher
            self._webview_launcher = WebViewLauncher(self)
        return self._webview_launcher

    def get_webview_manager(self) -> "WebViewManager | None":
        """获取 WebView 管理器"""
        if self._webview_launcher:
            return self._webview_launcher.get_manager()
        return None
